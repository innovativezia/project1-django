...
class UserManager():
    def render_message(self, user_data):
        ...
        if isinstance(user_data, dict):
            ...
            return (plain_, html_)
        return (None, None)
    def user_message(self, user_id=None, email=None, subject="Billing Update!"):
        user = self.get_user_data(user_id=None, email=None)
        if user:
            plain_, html_ = self.render_message(user)
            print(plain_, html_)
            user_email = self.get_user_data("email", "ziagulzia@gmail.com")
            ...
            try:
                ...
                he_msg['Subject'] = subject
                the_msg["From"] = from_email
                he_msg["To"]  = user_email
                part_1 = MIMEText(plain_, 'palin')
